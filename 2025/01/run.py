...
def convert_rotation(instruction) -> int:
    """Convert rotation to an positive (right) or negative (left) offset"""
...
with open(dir_path / "input") as f:
    ...
    count_zero_part1 = 0
    count_zero_part2 = 0
    # Each instruction contains an offset (positive or negative)
    for offset in instructions:
        # --- This is part 2: We care about landing on and crossing 0!
        ...
        if raw_current <= 0:
            ...
            count_zero_part2 += 1 if current != 0 else 0
    ...
    print(f"Part 1: Count zeros={count_zero_part1} (should be 1182)")
    print(f"Part 2: Count zeros={count_zero_part2} (should be 6907)")
